       ...
       const char** argv)
{
  ...
  for ( ; ; ++ p1) {
	...
	if ( popt_c.is_specified() ) {
	  ...
	  rcg1.generate(rg);
	  vector<Variable> vars_list;
	  for (ymuint j = 0; j < p1; ++ j) {
	    ymuint idx = rcg1.elem(j);
	    const Variable& var1 = var_list[idx];
	    vars_list.push_back(var1);
	  }
	  f = new SigFunc(vars_list);
	}
	ymuint nv = rv_mgr.vect_size();
	ymuint np1 = 1UL << p1;
	FuncVect* fv = new FuncVect(nv, np1);
	for (ymuint i = 0; i < nv; ++ i) {
	  const RegVect* rv = rv_mgr.vect_list()[i];
	  ymuint val = f->eval(rv);
	  fv->set_val(i, val);
	}
	fv_list[i] = fv;
	delete f;
	...
	sigfunc_list[i] = f;
      ...
      vector<ymuint> block_map;
      bool stat = pt.cf_partition(rv_mgr.vect_list(), sigfunc_list, block_map);
#endif
      for (ymuint i = 0; i < m; ++ i) {
